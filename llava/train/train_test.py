...
def train():
    ...
    elif "timdettmers/guanaco-13b-merged" in model_args.model_name_or_path:
        tokenizer = transformers.AutoTokenizer.from_pretrained(
            "huggyllama/llama-7b",
            cache_dir=training_args.cache_dir,
            model_max_length=training_args.model_max_length,
    ...
    if list(pathlib.Path(training_args.output_dir).glob("checkpoint-*")):
        ...
        ####################################
        # EVAL here before saving
        ####################################
        test_before_saving(model, tokenizer, vision_tower.image_processor, model_name=training_args.output_dir)
...
def test_before_saving(old_model, tokenizer, image_processor, model_name="testing_before_saving"):
    import pandas as pd
    from tqdm import tqdm
    from llava.mm_utils import KeywordsStoppingCriteria
    
    print('Merging LoRA weights...')
    model = old_model.merge_and_unload()
    ...
    input_file_path = "./data/datasets/llava_test.xlsx"
    ...
    for index, item in tqdm(enumerate(zip(dataframe['ID'].tolist(), dataframe['Description'].tolist(), 
                                          ...
                                          desc=f'Cycling {input_file_path} data..'):
        ...
        image_file_path = f"{image_path}/{game_id}.png"
        ...
        outputs = "I can execute it." if outputs == "Yes" else answer
    ...
    cm_df = compute_confusion_matrix(expected_outputs, outputs_list, task="classification")

    file_to_save = f"./data/predictions/{model_name}_classification.xlsx"
